...
fn compile() {
    ...
    let mut config = Config::new(dir);
    config.reconf("-fi");
    config.cflag(format!("-I {}/include -fPIC -gz=none", out_dir));
    config.cxxflag(format!("-I {}/include -fPIC -gz=none", out_dir));
    ...
    config.cflag(format!("-pthread -I {}/include -fPIC -gz=none", out_dir));
    config.ldflag(format!("-L{}/lib -latomic -ldl -gz=none", out_dir));
