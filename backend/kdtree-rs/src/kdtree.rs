...
impl<A: Float + Zero + One, T, U: AsRef<[A]>> KdTree<A, T, U> {
    ...
    pub fn in_range<'a>(&'a self, min: &[A], max: &[A]) -> Vec<&'a T> {
        let mut results = vec![];
        self.in_range_step(min, max, &mut results);
        return results;
    }

    fn in_range_step<'a>(&'a self, min: &[A], max: &[A], results: &mut Vec<&'a T>) {
        if self.is_leaf() {
            let points = self.points.as_ref().unwrap().iter();
            let bucket = self.bucket.as_ref().unwrap().iter();
            results
                .extend(points.zip(bucket).filter_map(|(p, d)| {
                    let point = p.as_ref();
                    for dim in 0..self.dimensions{
            ...
            if self.belongs_in_left(min) {
                if let Some(left) = &self.left {
                    left.in_range_step(min, max, results);
                }
            }
            if !self.belongs_in_left(max) {
                if let Some(right) = &self.right {
                    right.in_range_step(min, max, results);
                }
            }
        }
    }
