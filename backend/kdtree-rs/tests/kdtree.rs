...
fn range_search_works() {
    let dimensions = 2;
    let capacity_per_node = 10;
    let num_points = 10000;

    let mut kdtree = KdTree::with_capacity(dimensions, capacity_per_node);

    let range_min: [f64; 2] = [ -10.0, -5.0 ];
    let range_max: [f64; 2] = [ 3.0, 4.0 ];
    ...
    for point_index in 0..num_points {
        let mut point: [f64; 2] = [0.0; 2];
        let mut is_in_range = true;
        for dim in 0..dimensions {
            point[dim] = rng.gen_range(range_min[dim] - 5.0, range_max[dim] + 5.0);
            is_in_range &= range_min[dim] <= point[dim] && point[dim] <= range_max[dim];
        }

        kdtree.add(point, is_in_range).unwrap();
        if is_in_range {
            num_in_range += 1;
        }
    }
    // check that we don't have a trivial case
    assert_ne!(num_in_range, 0);
    assert_ne!(num_in_range, num_points);

    let in_range_result = kdtree.in_range(&range_min, &range_max);
    assert_eq!(in_range_result.len(), num_in_range);
    assert!(in_range_result.iter().all(|is_in| { return is_in.clone().clone(); }));
}