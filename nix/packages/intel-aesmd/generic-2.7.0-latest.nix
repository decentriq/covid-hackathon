...
in stdenv.mkDerivation {
  ...
  buildCommand = ''
    ...
    cat $out/lib/systemd/system/aesmd.service.orig \
     ...
     | sed "s:ExecStartPre=/bin/chmod 0755 /var/run/aesmd/:ExecStartPre=/bin/chmod 0755 /var/run/aesmd/\nExecStartPre=/bin/mkdir -p /var/opt/aesmd/data/:" \
     | sed "/Type=simple/a Environment=LD_LIBRARY_PATH=/opt/intel/libsgx-enclave-common/aesm:${lib.makeLibraryPath [ pkgs.openssl protobuf3_0 pkgs.curl ]}" \
